...
// Arguments: binary, grey file, /*ratio,*/ nthreads
#define ARGS 3
...
void *sample(void *p)
{
  ...
  do
  {
    ...
    {
#     ifdef DEBUG
      if (thread_id == 0 && misses % 100 == 0) printf("[DEBUG] CONSECUTIVE MISSES: %ld\n", misses);
#     endif
...
main(int argc, char **argv)
{
  ...
  num_cols = img_width / nthreads;
  num_cols += img_width % nthreads > 0 ? 1 : 0;
...
# endif
  ...
  {
    thread_id[i] = i;
    pthread_create(&threads[i], NULL, sample, &thread_id[i]);
  }
  for (i = 0; i < nthreads; i++)
  {
